...
public class Main_BJ_9663 {
   ...
   public static void main(String[] args) {
      ...
      N = scan.nextInt();
      col = new int[N + 1]; 
      //0번 행을 제외하고 작업하기 위해 1을 더한다.
      //각 행에 하나씩만 배치할 수 있기 때문에 1차원, col[i]번째방에 
      //Queen을 배치할 열값을 저장한다.
      setQueens(1); // 1행부터 시도
   ...
   // 유도파트 : 현재 행의 첫열부터 무조건 퀸을 놓고 그 위치가 가능한지 체크하고  
   // 가능하면 다음 행으로 퀸을 놓으러 재귀호출한다.
   ...
   public static void setQueens(int rowNo) {
      ...
      // ** 백트래킹 **
      for (int j = 1; j <= N; j++) { // 해당 행의 1열부터 n열까지 Queen을 위치시키는 시도
         col[rowNo] = j;
   ...
   // rowNo행에 퀸을 놓을수 있는지 1행부터 기존 rowNo-1행까지 rowNo행와 비교하며 유망함을 체크
   public static boolean checking(int rowNo) {
      for (int k = 1; k < rowNo; k++) {
         ...
         if (col[rowNo] == col[k] || Math.abs(col[rowNo] - col[k]) == rowNo - k) {
            // col[rowNo] == col[k] 열이 같은지 체크
            // (한행에 하나씩 구하기 때문에 행체크 불필요)
            return false; 
   // Math.abs(col[rowNo]-col[k]) == rowNo-k 대각선 체크 (열값차이와 행값차이가 같다면 대각선)
         }
      }
      return true;
   }
